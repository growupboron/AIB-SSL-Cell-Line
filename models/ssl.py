...
class BaseSSL(nn.Module):
    ...
    RXRX_PATH = os.environ.get('RXRX_PATH', '/work/cvcs_2023_group23/AIB/data/rxrx1_v1.0')
    RXRX_META_PATH = os.environ.get('RXRX_META_PATH', '/work/cvcs_2023_group23/AIB/data/rxrx1_v1.0/metadata.csv')
    ...
    def __init__(self, hparams):
        ...
        if hparams.data == 'rxrx':
            print(f"RXRX_PATH = {self.RXRX_PATH}")
            print(f"RXRX_META_PATH = {self.RXRX_META_PATH}")
        if hparams.data == 'rxrxfull':
            print(f"RXRX_PATH = {self.RXRX_PATH}")
            print(f"RXRX_META_PATH = {self.RXRX_META_PATH}")
    ...
    def prepare_data(self):
        ...
        elif self.hparams.data == 'rxrx':
            full_dataset = models.data.RxRx1Dataset(self.RXRX_META_PATH, self.RXRX_PATH, transform=train_transform)
        ...
        elif self.hparams.data == 'rxrxfull':
            full_dataset = models.data.RxRx1FullDataset(self.RXRX_META_PATH, self.RXRX_PATH, transform=self.transform)          
            # Calculate sizes of splits
            total_size = len(full_dataset)
            train_size = int(0.8 * total_size)
            val_size = int(0.1 * total_size)
    ...
    def add_model_hparams(cls, parser):
        parser.add_argument('--data', help='Dataset to use', default='rxrx')
        parser.add_argument('--arch', default='ResNet50', help='Encoder architecture')
        parser.add_argument('--batch_size', default=4, type=int, help='The number of unique images in the batch')
...
class SSLEval(BaseSSL):
    ...
    def __init__(self, hparams, device=None):
        ...
        self.encoder.eval()
        #torch.ones(10, 3, 32, 32) creates a tensor with dimensions [batch_size, channels, height, width]. The value 10 represents the batch size.
        if hparams.data == 'cifar':
            ...
            n_classes = 1000
        elif hparams.data == 'rxrx':
            hdim = self.encode(torch.ones(10, 3, 256, 256).to(device)).shape[1]
            n_classes = 4
        elif hparams.data == 'rxrxfull':
            hdim = self.encode(torch.ones(10, 3, 256, 256).to(device)).shape[1]
            n_classes = 1138
    ...
    def transforms(self):
        if self.hparams.data == 'cifar':
            ...
            test_transform = transforms.Compose([
                datautils.CenterCropAndResize(proportion=0.875, size=256),
                transforms.ToTensor(),
                lambda x: (255 * x).byte(),
            ])
        elif self.hparams.data == 'rxrxfull':
...
class SemiSupervisedEval(SSLEval):
    ...
    def prepare_data(self):
        ...
        if len(self.trainset) != self.hparams.train_size:
            ...
            if self.hparams.data == 'cifar' or self.hparams.data == 'cifar100' or self.hparams.data == 'rxrx' or self.hparams.data == 'rxrxfull':
