...
def find_free_port():
    ...
    _, port = s.getsockname()  # Get the port assigned by the operating system
...
def add_learner_params(parser):
    ...
    # parallelizm params:
    parser.add_argument('--dist', default='ddp', type=str,
        help='dp: DataParallel, ddp: DistributedDataParallel',
        choices=['dp', 'ddp'],
    )
    # parser.add_argument('--dist_address', default='127.0.0.1:40283', type=str,
    #     help='the address and a port of the main node in the <address>:<port> format'
    # )

    parser.add_argument('--dist_address', default=f'127.0.0.1:{find_free_port()}', type=str,
        help='the address and a port of the main node in the <address>:<port> format'
    )
...
def generate_experiment_name(args):
    # Here, you can generate a unique experiment name based on parameters or timestamp
    # For example, concatenate parameters or use the current timestamp
    experiment_name = "experiment_" + time.strftime("%Y-%m-%d_%H-%M-%S")
...
def main_worker(gpu, ngpus, args):
    ...
    tb_root_dir = 'tblogs'
    experiment_dir = os.path.join(tb_root_dir, experiment_name)
    os.makedirs(experiment_dir, exist_ok=True)

    # Modify the log directory to save logs for the current experiment
    logger = Logger('logs', base=experiment_dir, fmt=fmt)
    ...
    while continue_training:
        ...
        for _, batch in enumerate(train_loader):
            ...
            if not args.eval_only:
                ...
                optimizer.step()
                print("4")
                writer.add_scalar('Train Loss / Iteration', loss.item(), cur_iter)
                writer.add_scalar('Train Loss / Batch', loss.item(), batch)                
